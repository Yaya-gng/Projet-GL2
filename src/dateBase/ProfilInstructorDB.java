...
public static void Inscrire(String nom, String prenom, Date date, String adresse, String grade,String sp,String password) {
		
		try(Connection con = connect();
		PreparedStatement pr = con.prepareStatement("Insert into instructeur(nom,prenom,dateNaiss,adresse,grade,specialite,password)values(?,?,?,?,?,?,?)");)
		{
		pr.setString(1,nom);
		pr.setString(2,prenom);
		pr.setDate(3,date);
		pr.setString(4,adresse);
		...
		PreparedStatement pr1 = con.prepareStatement("Insert into user(nom,prenom,password,instructeur)values(?,?,?,?)");
